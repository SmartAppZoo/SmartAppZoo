...
def deviceCapabilityList(device) {
    def i = 0
    device.capabilities.collectEntries {
        capability ->
            [
                (capability.name): 1
            ]
...
def deviceCommandList(device) {
    def i = 0
    device.supportedCommands.collectEntries {
        command ->
            [
...
def deviceAttributeList(device) {
    device.supportedAttributes.collectEntries {
        attribute ->
    	try {
            [
...
def changeHandler(evt) {
    ...
    if (state.directIP != "") {
        ...
        def deviceData = [device: evt.deviceId, attribute: evt.name, value: evt.value, date: evt.date, app_id: app.id]
        //How do I control the port?!?
        log.debug "Sending Update to ${state.directIP}:${state.directPort}"
        def bodyJson = new groovy.json.JsonOutput().toJson(deviceData)
