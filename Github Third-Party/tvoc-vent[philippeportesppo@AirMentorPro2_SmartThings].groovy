...
preferences { 
	section("TVOC Sensor") {
    input "Air_Mentor_Pro_2", "capability.carbonDioxideMeasurement", title: "TVOC Sensor", required: false
	input "level", "enum", title: "TVOC Level for vents action", options: ["good","moderate","unhealthy sensitive persons", "unhealthy", "very unhealthy"], required: true, multiple: true
}
...
def initialize() {
    ...
    subscribe(Air_Mentor_Pro_2, "TVOC", handleLevel)
}
def handleLevel(evt) {
    if (evt.name=="TVOC" && level.contains(evt.value)) {
