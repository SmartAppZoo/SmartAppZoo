/**
 *	Dimmable Button Controller App for ZWN-SC7
 *
 *	Author: Dieter Rothhardt based on: Matt Frank based on VRCS Button Controller by Brian Dahlem, based on SmartThings Button Controller
 ...
 */
definition(
    name: "ZWN-SC7	Dimmable Button Controller",
    namespace: "dasalien",
    author: "Dieter Rothhardt, based on code from Matt Frank and Brian Dahlem",
    description: "ZWN-SC7	 7-Button	 Scene	 Controller	Dimmable Button Assignment App",
...
def getButtonSections(buttonNumber) {
  return {
	...
	section(title: "Dim or Speed up these ...", hidden: hideSection(buttonNumber, "up"), hideable: true) {
		input "dimmer_${buttonNumber}_pushed", "capability.switchLevel", title: "Pushed", multiple: true, required: false
	}
	section(title: "Dim or Speed down these ...", hidden: hideSection(buttonNumber, "down"), hideable: true) {
		input "dimmerlower_${buttonNumber}_pushed", "capability.switchLevel", title: "Pushed", multiple: true, required: false
	}
	section(title: "Remember or restore the values of these light and fan swtiches ...", hidden: hideSection(buttonNumber, "restore"), hideable: true) {
		input "restorelight_${buttonNumber}_pushed", "capability.switchLevel", title: "Light", multiple: false, required: false
		input "restorefan_${buttonNumber}_pushed", "capability.switchLevel", title: "Fan", multiple: false, required: false
    }
...
def initialize() {
    ...
    state.lightValuesStored = false
    state.fanValuesStored = false
    state.lightLevel = 0
    state.fanLevel = 0
...
def buttonConfigured(idx) {
	log.debug "buttonEvent  $idx"
  return settings["lights_$idx_toggle"] ||
    settings["locks_$idx_toggle"] ||
    settings["sonos_$idx_toggle"] ||
	settings["dimmer_$idx_pushed"] ||
    settings["dimmerlower_$idx_pushed"] ||
	settings["restorelight_$idx_pushed"] ||
    settings["restorefan_$idx_pushed"] ||
...
def buttonEvent(evt){
  ...
  if(allOk) {
      ...
      //Dieter: not sure what this is to accomplish...
    ...
    //Dieter } else if (firstEventId == 0) {
    //Dieter   log.debug "No events found. Possible SmartThings latency"
    //Dieter } else {
    //Dieter   log.debug "Duplicate button press found. Not executing handlers"
    //Dieter }
...
def executeHandlers(buttonNumber, value) {
  ...
  if (lights != null) toggle(lights)
	...
	def dimmer = find('dimmer', buttonNumber, "pushed")
	if (dimmer != null) dimup(dimmer)

	def dimmerlower = find('dimmerlower', buttonNumber, "pushed")
	if (dimmerlower != null) dimdown(dimmerlower)

	def sr_light = find('restorelight', buttonNumber, "pushed")
	if (sr_light != null) save_restore_light(sr_light)

	def sr_fan = find('restorefan', buttonNumber, "pushed")
	if (sr_fan != null) save_restore_fan(sr_fan)
...
def find(type, buttonNumber, value) {
  ...
  log.debug "Found: $preferenceName"
  def pref = settings[preferenceName]
  log.debug "Found: $pref"
  if(pref != null) {
...
def dimup(devices) {
	...
	def dimlevel = devices*.currentValue('level')
    def step = 5;
    def newlevel = dimlevel.getAt(0) + step
...
def save_restore_light(devices) {
    if(state.lightValuesStored ) {
    	// Restore values
		def lightlevel = state.lightLevel
        devices.setLevel(lightlevel)
    	state.lightValuesStored = false
    } else {
    	// Remember values
		def dimlevel = devices*.currentValue('level')
		def lightlevel = dimlevel.getAt(0)
		state.lightLevel = lightlevel
        devices.off()
        state.lightValuesStored = true
    }
}

def save_restore_fan(devices) {
    if(state.fanValuesStored ) {
    	// Restore values
	    def fanlevel = state.fanLevel
        devices.setLevel(fanlevel)
    	state.fanValuesStored = false
    } else {
    	// Remember values
		def dimlevel = devices*.currentValue('level')
	    log.debug "dimlevel  ${dimlevel}"
		def fanlevel = dimlevel.getAt(0)
	    state.fanLevel = fanlevel
        
        state.fanValuesStored = true
    }
}
