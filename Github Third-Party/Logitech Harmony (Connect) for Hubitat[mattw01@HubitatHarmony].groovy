...
preferences(oauthPage: "deviceAuthorization") {
    ...
    page(name: "manualAuthEntry")
    page(name: "completeManualAuth")
    page(name: "authPage")
}
...
def authPage() {
	...
	if (!state.HarmonyAccessToken) {
        ...
        return dynamicPage(name: "Credentials", title: "Harmony", nextPage: null, uninstall: true, install:false) {
            section { paragraph title: "Note:", "Authenication information must be manually entered from SmartThings for now." }
            section { href "manualAuthEntry", required:true, title:"Manual Authenication", description:"Continue to enter authenication details" }
        }
    } else {
        ...
        log.debug "HarmonyHubs: ${state.HarmonyHubs}"
		log.debug "HarmonyActivities: ${state.HarmonyActivities}"
        ...
        def hubMap = ["NoHub" : "No Hub"]
        if(huboptions) hubMap << huboptions
        def actMap = ["NoAct" : "Blank Activity"]
        if(actoptions) actMap << actoptions
		...
		return dynamicPage(name:"Credentials", title:"Discovery Started!", nextPage:"", refreshInterval:refreshInterval, install:true, uninstall: true) {
			section("Please wait while we discover your Harmony Hubs and Activities. Discovery can take five minutes or more, so sit back and relax! Select your device below once discovered.") {
                input(name: "selectedhubs", title:"Select Harmony Hubs (${numFoundHub} found)", type: "enum", required:false, multiple:true, description: "Tap to choose", options:hubMap, submitOnChange: true)  
				//input("selectedhubs", title:"Select Harmony Hubs (${numFoundHub} found)", type: "enum", required:false, multiple:true, submitOnChange: true, options: huboptions)
			}
      	...
      	if (numFoundHub > 0 && numFoundAct > 0 && true)
			section("You can also add activities as virtual switches for other convenient integrations") {
				input "selectedactivities", "enum", required:false, title:"Select Harmony Activities (${numFoundAct} found)", multiple:true, submitOnChange: true, options:actMap
			}
            section("") {
...
def manualAuthEntry() {
	 return dynamicPage(name: "manualAuthEntry", title: "Manually enter authenication details", nextPage: "completeManualAuth") {
         section("Manual Authenication Details") {
             paragraph "Enter authenication details from SmartThings below"
         	input "manualAccessToken", "text", title: "Harmony Access Token (HarmonyAccessToken)", required: true 
         	input "manualAuthToken", "text", title: "Access Token (accessToken)", required: true 
         }
     }
    
}
def completeManualAuth() {
    log.debug "Entering completeManualAuth()"
    if(manualAccessToken && manualAuthToken) {
        log.debug "completeManualAuth() settings tokens"
        state.HarmonyAccessToken = manualAccessToken
        state.accessToken = manualAuthToken
         return dynamicPage(name: "completeManualAuth", title: "Manual Authenication Complete") {
             section("Authenication Sucessfully Added") {
             	paragraph "All done! Authenication details have been saved."
                href "authPage", title: "Complete and return to setup"
             }
         }
    }
    else {
         return dynamicPage(name: "completeManualAuth", title: "Manual Authenication Error") {
             section("Authenication Error") {
             	paragraph "No authenication details were saved. Please go back and try again"
                section { href "manualAuthEntry", required:true, title:"Manual Authenication", description:"Continue to enter authenication details" }
             }
    	}
	}
