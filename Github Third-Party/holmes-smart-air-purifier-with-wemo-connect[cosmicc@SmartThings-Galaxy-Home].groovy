/**
 *  WeMo Air Purifier Service Manager
 *
 *  Author:  Ian Perry, Brian Keifer, heavily based on code by Kevin Tierney
 *  Version: 0.0.1
 *  Date:    2016-12-25
 */

definition(
    name: "Holmes Smart Air Purifier With WeMo (Connect)",
    namespace: "cosmicc",
    author: "cosmicc",
    description: "Allows you to integrate your Holmes WeMo Air Purifier with SmartThings.",
    category: "My Apps",
...
log.trace "humidifiersDiscovered()"
    def humidifiers = getWemoHumidifiers().findAll { it?.value?.verified == true }
    log.trace "a"
    def map = [:]
    humidifiers.each {
...
def addHumidifiers() {
    ...
    selectedHumidifiers.each { dni ->
        ...
        if (!d) {
            log.debug "Creating WeMo Air Purifier with dni: ${selectedHumidifier.value.mac}"
            log.debug "IP: ${selectedHumidifier.value.ip} - PORT: ${selectedHumidifier.value.port}"
            d = addChildDevice("bkeifer", "Holmes Smart Air Purifier With WeMo", selectedHumidifier.value.mac, selectedHumidifier?.value.hub, [
                "label": selectedHumidifier?.value?.name ?: "WeMo AirPurifier",
                "data": [
...
log.debug("PARSED: ${parsedEvent}")
    if (parsedEvent?.ssdpTerm?.contains("Belkin:device:AirPurifier")) {
        log.debug("FOUND AIR PURIFIER")
        def humidifiers = getWemoHumidifiers()

        if (!(humidifiers."${parsedEvent.ssdpUSN.toString()}"))
        { //if it doesn't already exist
            log.debug ("Creating air purifiers")
