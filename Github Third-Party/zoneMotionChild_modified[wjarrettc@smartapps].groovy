/*
    ...
    2.1.1a  2017-03-13  Added support for Override State Change triggers and Locks as triggers (by W. Jarrett Campbell)
...
def initialize() {
    ...
    state.zoneOverrideActive = false
...
def anyTriggersActive(evtTime){
    ...
    if (!enable && triggerLocks){
    	enable = triggerLocks.any{ s -> s.statesSince("lock", evtStart).size > 0}
        //log.debug "triggerAccel:${state}"
    }
...
//Override Activation
def anyOverrideActive(){
	def enable = false

    if (triggerMotions){
    	enable = triggerMotions.any{ s -> s.currentMotion == "active"}
        log.debug "Any override contact closed? ${enable}"
    }
    
    if (!enable && triggerContacts){
    	enable = triggerContacts.any{ s -> s.currentContact == "closed"}
        log.debug "Any override contact closed? ${enable}"
    }

    if (!enable && triggerSwitches){ 	
    	enable = triggerSwitches.any{s -> s.currentSwitch == "on"}
        log.debug "Any override switch on? ${enable}"
    }
    
    if (!enable && triggerAccel){
    	enable = triggerSwitches.any{s -> s.currentSwitch == "active"}
        log.debug "Any override acceleration sensor active? ${enable}"
    }
    
    if (!enable && triggerLocks){
    	enable = triggerLocks.any{ s -> s.currentLock == "unlocked"}
        log.debug "Any override lock unlocked? ${enable}"
    }
    
    if (!enable) log.trace "No overrides were detected."
			...
			case "2":
             ...
             case "3":
        		if (!anyOverrideActive()){
            		zoneOn()
                    log.debug "No overrides active, turning zone on"
                	activityTimeoutHandler(evtTime,device)
            	} else {
            		activityTimeoutHandler(evtTime,device)
                    log.debug "Zone not activated due to override"
            	}
	        	break
 		}
    } else {
...
def inactiveHandler(evt){
	if (allInactive()){
    	if (settings.zoneType == "0") zoneOff()
        if (settings.zoneType == "3" && !anyOverrideActive()) zoneOff()
    }
        ...
        ,uninstall	: installed
		     ...
		     section(){
                    ...
                    input(
                        ...
                        ,multiple				: false
                        ,required				: true
                        ,options				: [[0:"False Motion Reduction"],[1:"Motion Aggregation"],[2:"Triggered Activation"],[3: "Override State Change"]]
                        ,submitOnChange			: true
                    )
            }
            if (zType){
                section(){
                    ...
                    if (zType == "0"){
                     ...
                     if (zType == "2" || zType == "3"){
                     	href(
                        	name		: "dv"
                        	,title		: "Trigger or Override Devices" 
                        	,required	: false
                        	,page		: "triggers"
                        	,description: null
...
def triggerPageComplete(){
	if (triggerMotions || triggerContacts || triggerSwitches || triggerAccel || triggerLocks){
    	return "complete"
    } else {
    	return null
    }
...
def getDescription(zType){
   switch (zType) {
		...
		case "2":
            ...
            break
        case "3":
        	return "Zone does not change state when override devices are in pre-defined state." +
