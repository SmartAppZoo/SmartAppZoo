...
@Field CAPABILITY_MAP = [
    ...
    "switches": [
        ...
        attributes: [
            //"382b78054f7a,plug,on/set"
            "switch/state",
            "switch",
            "switch/state/set",
            //"382b78054f7a,switch/state",
            //"382b78054f7a,switch",
            //"382b78054f7a,switch/state/set"
        ],
        action: "actionOnOff"
    ],
    "soundPressureLevel": [
...
def getDeviceNames(devices) {
    ...
    devices.each{device->
        ...
        list.push(device.preferences.MACAddr)
    }
    list
}
    ...
    CAPABILITY_MAP.each { key, capability ->
        capability["attributes"].each { attribute ->
            ...
            settings[key].each {device ->
                ...
                attributes[attribute].push(device.preferences.MACAddr)
            }
        }
    }
    ...
    CAPABILITY_MAP.each { key, capability ->
        if (capability["attributes"].contains(json.type)) {
            settings[key].each {device ->
                ...
                if (device.preferences.MACAddr == json.name) {
...
def inputHandler(evt) {
   ...
   //def evtData = new JsonSlurper().parseText(evt.deviceId)
   //log.debug "MacAddr:  ${evt.device.preferences.MACAddr}"  //tempSensor.currentState("temperature")
   //log.debug evt.device.metadata["preferences"]["sections"]["input"]
   //String[] mqttinfo = evt.name.split(",")
   //String macaddr = mqttinfo[0]
   //String command = mqttinfo[1]
   //log.debug macaddr
   //log.debug devtype
   //log.debug command
    ...
    def json = new JsonOutput().toJson([
        ...
        body: [
            ...
            //name: evt.device.deviceNetworkId,  //This works using Device Network ID
            name:  evt.device.preferences.MACAddr,		//This works using MACAddr Custom Preference
            //name: evt.device.MacAddr,
            //name: macaddr,
            value: evt.value,
            //type: command
            type: evt.name
...
def actionPressed(device, attribute, value) {
    if (value == "pressed") {
       device.pressed()
    }
}

def actionSetLevel(device, attribute, value) {
    //TRACE( "MQTT device set ${value}" )
    log.debug "Value ${value}"
    device.setLevel(value)
}

def actionsetHumidityLevel(device, attribute, value) {
    //TRACE( "MQTT device set ${value}" )
    log.debug "Value ${value}"
    device.setHumidityLevel(value)
}
