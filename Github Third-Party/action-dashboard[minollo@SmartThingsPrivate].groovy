...
def command() {
    ...
    def endState
    def attribute
    log.debug "request is value: ${value}, type: ${type}"
    if (value == "toggle" && type == "presence") {
	log.debug "toggle presence"
        device = presence?.find{it.id == id}
	log.debug "toggle presence; device: ${device}"
        if (device) {
            attribute = "presence"
            if(device.currentValue(attribute) == "present") {
                endState = "not present"
            } else {
                endState = "present"
            }
            device.togglePresence()
        }
    } else if (value == "toggle" && (type == "dimmer" || type == "switch")) {
...
def initialize() {
    ...
    runEvery1Hour(scheduledWeatherRefresh)
    getURL(null)
}
...
def data() {
    [
    	alarms: alarms?.collect{[type: "alarm", id: it.id, name: it.displayName, status: it.currentValue('alarmStatus'), enabled: it.currentValue('enabled')]}?.sort{it.name},
        ...
        thermostat: thermostats?.collect{[type: "thermostat", id: it.id, name: it.displayName,
        	status: roundNumber(it.currentValue('temperature'), "°"),
        	target: roundNumber(it.currentValue('thermostatSetpoint'), "°"),
            targetType: (it.currentValue('thermostatMode')=="heat" || it.currentValue('thermostatMode')=="emergencyHeat")?"heat":(it.currentValue('thermostatMode')=="cool"?"cool":"off"),
        	activity: it.currentValue('thermostatOperatingState')=="heating"?"heating":(it.currentValue('thermostatOperatingState')=="cooling"?"cooling":"idle"),
        ]}?.sort{it.name},
...
<script>
    ...
    function animatePresence(element) {
        var oldIcon = element.find(".st-icon i").attr("class");
        var newIcon = toggleIcon(oldIcon);
        spinOn(element).animate({opacity: .5}, 300).find(".st-icon i").removeClass(oldIcon).addClass(newIcon).closest(element).animate({opacity: 1}, 200);
	return element;
	...
	"fa fa-map-marker" : "fa opaque fa-map-marker",
	"fa opaque fa-map-marker" : "fa fa-map-marker"
        }
...
.lock, .switch, .dimmer, .momentary, .presence {
...
<div id="mode_mode" class="st-tile mode">
	<div class="st-tile-content">
        ...
        <div class="st-icon" id="mode-icon">
			<i class="fa fa-home" style="${(mode != "Home" && mode != "Day") ? "display:none" : ""}" id="mode-home"></i>
            <i class="fa fa-moon-o" style="${!mode.contains("Night") ? "display:none" : ""}" id="mode-night"></i>
            <i class="fa fa-bed" style="${!mode.contains("Sleep") ? "display:none" : ""}" id="mode-name"></i>
            <i class="fa fa-sign-out" style="${mode != "Away" ? "display:none" : ""}" id="mode-away"></i>
        </div>
        <div class="st-icon" style="${(mode in ["Home", "Away", "Day"] || mode.contains("Night") || mode.contains("Sleep")) ? "display:none;": ""} font-size:1.25em; height:2em;line-height:2em; margin-top:-1em;" id="mode-name">$location.mode</div>
        ${mode.contains("alarm") ? "<div class=\"st-icon\" id=\"overlay-icon\" style=\"color: red;left: 65%;top: 60%;font-size: 6px;\"><i class=\"fa fa-bell-o\"></i></div>" : ""}
...
<div id="temperature_$device.id" class="st-tile temperature">
	<div class="st-tile-content">
        ...
        <div class="st-icon" ${device.activity=="heating"?"style=\"color: red\"":(device.activity=="cooling"?"style=\"color: blue\"":"")}>
        	$device.status
        </div>
        <div class="footer" ${device.targetType=="heat"?"style=\"color: red\"":(device.targetType=="cool"?"style=\"color: blue\"":"style=\"color: gray\"")}><i class="wi wi-thermometer"></i></div>
        <div class="footer" style="font-size: 12px; left:20px; bottom:8px">Target: $device.target</div>
	</div>
</div>
"""
}
...
def renderAlarm(device) {
"""
<div class="st-tile alarm">
	<div class="st-tile-content"  ${device.status == "away"?"style=\"background:#da532c\"":(device.status == "stay"?"style=\"background:#f0801a\"":"style=\"background:#99cc00\"")}>
    	<div class="st-title">
            $device.name
        </div>
        <div class="st-icon">
        	<i class="fa ${device.enabled == "false" ? "opaque" : ""} ${(device.status == "away" || device.status == "stay") ? "fa-lock" : "fa-unlock-alt"}"></i>
        </div>
        <i class="spin fa fa-refresh fa-spin"></i>
	</div>
</div>
...
"""
<div id="presence_$device.id" class="st-tile presence" deviceId="$device.id" deviceType="presence">
	<div class="st-tile-content">
    	<div class="st-title">
        	$device.name
        </div>
...
def renderDevice(device) {
    ...
    if (device.type == "alarm") return renderAlarm(device)
    ...
    if (device.type == "thermostat") return renderThermostat(device)
