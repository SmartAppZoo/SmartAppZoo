 ...
 *  Humidity Alert
 *
 *  Copyright 2015 Brian Critchlow
...
def humidityHandler(evt) {
	...
	if (currentHumidity > tooHumid) {
		...
		if (alreadySentSms) {
			...
			send("${humiditySensor1.label} sensed humidity rose above $humidity1%. Current level is ${evt.value}%.")
			//switch1?.on()
		}
	}
    ...
    if (currentHumidity < notHumidEnough) {
		...
		if (alreadySentSms) {
			...
			log.debug "Humidity Fell Below ${notHumidEnough}:  sending SMS to $phone1 "
			send("${humiditySensor1.label} sensed humidity fell below $humidity2% threshold. Current level is ${evt.value}%.")
			//switch1?.off()
		}
	}
}
