/**
 *  Simple Event Logger - SmartApp v 1.6
 *
 *  Author: 
...
def installed() {	
    ...
    //Ver 1.6: Initialize time for last event
    if (state.firstEventTime == null) {state.firstEventTime = new Date().time}
}
...
def logNewEvents() {	
	...
	// Move the date range to the next position unless the google script failed last time or was skipped due to the sheet being archived and status.eventsLogged > 0.
    ...
    // Ver 1.6:
    status.firstEventTime = getFirstEventTimeMS(state.firstEventTime + 1000)
	//status.firstEventTime = getFirstEventTimeMS(status.lastEventTime); Deleted Ver 1.5
	status.lastEventTime = getNewLastEventTimeMS(status.started, (status.firstEventTime + 1000))
    
	def startDate = new Date(status.firstEventTime)
...
private getArchiveOptions() {
	return [
		...
		interval: safeToLong(settings?.archiveInterval, 1500000)
	]
}
...
private getNewEvents(startDate, endDate) {	
	...
	getSelectedDevices()?.each  { device ->
		getDeviceAllowedAttrs(device?.displayName)?.each { attr ->
			...
			//Ver 1.6: capture last event time for startDate of next getNewEvents() period
            if (event.date.time > state.firstEventTime) {state.firstEventTime = event.date.time}
            //log.debug "startDate= $startDate; event.date?.time = $event.date; state.firstEventTime = $state.firstEventTime"
            }
		}
	}
...
private getFormattedLocalTime(utcTime) {
	if (utcTime) {
		try {
            ...
            //return localDate.format("dd/MM/yyyy HH:mm:ss") // formato fecha para españa
		}
		catch (e) {
