 ...
 *
 *	7 Button Controller for HLGS
 *
 *	Author: Anthony Pastor (based closely on SmartThings's original Button Controller App)
 *	Date: 2016-5-4
 */
definition(
    name: "7 Button Controller for HLGS",
    namespace: "infofiend",
    author: "Anthony Pastor",
    description: "Control devices (including HLGS Scenes) with up to 7 buttons like the Aeon Labs Minimote",
...
def configureButton5() {
	dynamicPage(name: "configureButton5", title: "If you have a fifth button, you rule, and can set it up here",
		install: true, uninstall: true, getButtonSections(5))
}
...
def getButtonSections(buttonNumber) {
	return {
		section("Lights (any type) - simple toggle ON / OFF") {
        ...
        section("HLGS - Scene ON / Lights or Groups OFF (Requires BOTH of the following preferences to be set)") {
			input "hlgs1_${buttonNumber}_pushed", "capability.momentary", title: "First Push of this Button calls this HLGS Scene (momentary device)", multiple: false, required: false
			input "hlgs2_${buttonNumber}_pushed", "capability.switch", title: "Second Push of this Button turns off these HLGS Lights and Groups", multiple: true, required: false
		}       
		section("Locks") {
...
def buttonConfigured(idx) {
	return settings["lights_$idx_pushed"] ||
        ...
        settings["hlgs1_$idx_pushed"] ||
        settings["hlgs2_$idx_pushed"]
}
...
def executeHandlers(buttonNumber, value) {
	...
	def hlgs1 = find('hlgs1', buttonNumber, value)	
	def hlgs2 = find('hlgs2', buttonNumber, value)
	if (hlgs1 != null && hlgs2 != null) {
		if (state.hlgs != true) {
    	    hlgs1.push()
			state.hlgs = true
	    } else {
