 ...
 */
definition(
    name: "Robot Vacuum Fixed",
    namespace: "shinznatkid",
    author: "Natkid Shinz",
    description: "Robot Vacuum Fixed",
    category: "My Apps",
...
def update() {
	...
	if (validateCommand(device, type, command)) {
    	if (command == "on") {
        	device.on()
        	device.setRobotCleanerCleaningMode("auto")
        	device.setRobotCleanerMovement("cleaning")
        }
        else if (command == "off") {
        	device.setRobotCleanerCleaningMode("stop")
            device.setRobotCleanerMovement("homing")
        }
        else {
			device."$command"()
        }
