...
/*
  ...
  def deviceDisplayName = it.displayName
  def device_state = [ label: it.name, type: type, id: it.id, network: deviceDisplayName ]
/*
  theAtts.each {att ->
  	device_state = [ label: it.name, type: type, id: it.id, network: att.name ]
  }
*/
...
def updated() {
	// subscribe(switches, "switch.open", contactOpenHandler)
}
/*
