 ...
 *
 *	Automation Director version 1.0
 *
...
def Triggers() {
	dynamicPage(name: "Triggers") {
		...
		section(title: "Restrict when to run...", hidden: hideOptionsSection(), hideable: true) {
			...
			input "Illuminance_Required", "capability.illuminanceMeasurement", title: "Illuminance sensor(s) above or below threshold", required: false, submitOnChange: true, multiple: false
			if (Illuminance_Required) {
				input "Illuminance_RequiredGreaterThan", "number", title: "Only when lux is >=", required: false, range: "1..*", submitOnChange: true
				input "Illuminance_RequiredLessThan", "number", title: "Only when lux is <=", required: false, range: "1..*", submitOnChange: true
		...
		if (Button) {
			section(title: "Configure Buttons...", hidden: true, hideable: true) {
				for ( def h = 0; h < Button.size(); h++) {
                    ...
                    //settings["NumberofButtons${Button[h].id}"] = Button[h].currentValue("numberOfButtons")
                    //paragraph "NumberofButtons = ${settings["NumberofButtons${Button[h].id}"]}"
					input "NumberofButtons${Button[h].id}", "number", title: "Number of Buttons on ${Button[h]}", required: true, submitOnChange: true//, defaultValue: Button[h].currentValue("numberOfButtons")
					input "Holdable${Button[h].id}", "bool", title: "Controller supports holding buttons", submitOnChange: true//, defaultValue: HoldDef
					if (settings["NumberofButtons${Button[h].id}"] != null) {
						...
						for ( def i = 1; i <= Num; i++) {
							...
							if (settings["Holdable${Button[h].id}"]) {input "${Button[h].id}Button${i}Hold", "enum", title: "${Button[h]} button $i Long Push", options: ActionstoTake, required: false, multiple: true}
						}
					}
				}
			}
...
def DoButtonHeld(evt) {
	if(allOk) {
		...
		if (settings["${Number}Button${buttonNumber}Hold"]) {
			//log.debug "Device that triggered the event: ${Changed.displayName} | Number = $Number | Button $buttonNumber Pushed | Command = ${settings["${Number}Button${buttonNumber}Push"]}"
...
private hideOptionsSection() {
	(starting || ending || days || modes || startingX || endingX || Illuminance_Required) ? false : true
}
