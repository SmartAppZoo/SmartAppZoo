/**
 *  Copyright 2017 Eaton Consulting
 *
 ...
 *
 *  Last Modification:
 *      SE - Clean up of old ecobee template code.
 */
...
definition(
		name: "Nest (Connect)",
		namespace: "NetworkGod/Nest",
		author: "Sidney Eaton",
		description: "Connect your Nest Protect to SmartThings.",
		category: "SmartThings Labs",
		//iconUrl: "https://s3.amazonaws.com/smartapp-icons/Partner/ecobee.png",
		//iconX2Url: "https://s3.amazonaws.com/smartapp-icons/Partner/ecobee@2x.png",
		singleInstance: true
) {
	appSetting "clientId"  //These should be the app Client ID on Nest
    appSetting "clientSecret"  //This should equal the client secret on Nest developers
}

preferences {
	page(name: "auth", title: "Nest", nextPage:"", content:"authPage", uninstall: true, install:true)
}
...
def authPage() {
	...
	if (!oauthTokenProvided) {
		return dynamicPage(name: "auth", title: "Login", nextPage: "", uninstall:uninstallAllowed) {
			section() {
				paragraph "Tap below to log in to the Nest service and authorize SmartThings access. Be sure to scroll down on page 2 and press the 'Allow' button."
				href url:redirectUrl, style:"embedded", required:true, title:"Nest", description:description
			}
		}
	} else {
		...
		return dynamicPage(name: "auth", title: "Select Your Nest Protect devices", uninstall: true) {
			section("") {
				paragraph "Tap below to see the list of Nest Protect devices available in your Nest account and select the ones you want to connect to SmartThings."
				input(name: "thermostats", title:"Select Your Nest Protect", type: "enum", required:true, multiple:true, description: "Tap to choose", metadata:[values:stats])
			}
		}
	}
}
...
def getEcobeeThermostats() {
    ...
    def params = [
      ...
      //uri: "https://developer-api.nest.com/devices/smoke_co_alarms/${device.deviceNetworkId}",
	...
	try {
		httpGet(params) { resp ->
            log.debug "Retrieving Nest Protect Devices....Status Code:" + resp.status + " DATA: " + resp.data
			if (resp.status == 200) {
                resp.data.each { stat -> 
                    log.debug stat
                    log.debug "Now parsing Nest Protect: " + stat.getKey() + " - " + stat.getValue().name
                    stats[stat.getKey()] = stat.getValue().name
...
def initialize() {
	...
	def devices = thermostats.collect { dni ->
		...
		if(!d) {
			d = addChildDevice(app.namespace, getChildName(), dni, null, ["label":"${atomicState.thermostats[dni]}" ?: "Nest Protect"])
...
// This is created by Sidney
//def getChildData(DeviceID) {
//  return atomicState.thermostats[DeviceID]
//}
...
private void storeThermostatData(thermostats) {
    log.trace "Storing Nest Protect data: $thermostats"
    def data
    atomicState.thermostats = thermostats.inject([:]) { collector, stat ->
        ...
        data = [
            battery_health: stat.getValue().battery_health,
            smoke_alarm_state: stat.getValue().smoke_alarm_state,
            co_alarm_state: stat.getValue().co_alarm_state,
            name: stat.getValue().name,
            is_online: stat.getValue().is_online,
            is_manual_test_active:stat.getValue().is_manual_test_active
        ]

        collector[dni] = [data:data]
        return collector
    }
    log.debug "updated ${atomicState.thermostats?.size()} Nest Protects: ${atomicState.thermostats}"
}
