...
preferences {
  ...
  section("Notification events (optional):") {
    input "notifyEvents", "enum", title: "Which Events?", description: "default (none)", required: false, multiple: true,
     options:
      ['all','alarm','armed','armed_stay','armed_away']
  }
}
...
//    if (paneldevice) {
//        subscribe(paneldevice, "dscpartition", myEventHandler, [filterEvents: false])
//    }
//}

//def myEventHandler(evt) {
//	log.debug "${evt.name} is ${evt.value}"
//    notifyEvents.each { notifyEvent->
//      if( notifyEvent == "all" || notifyEvent == evt.name ) {
//        def ntfMsg = "${evt.name} is ${evt.value}"
//        sendMsg( ntfMsg )
//      }
//    }
//}
...
private updateZoneDevices(zonedevices,zonenum,zonestatus) {
  ...
  if (zonedevice) {
    ...
    // TODO - Look at checking if the state changed before calling zone()...
    
    
    def curState = ""
    def newState = ""
    if( zonedevice.hasAttribute("motion") )
    {
       def eventMap = [
         'open':"active",
         'closed':"inactive",
       ...
       newState = eventMap."${zonestatus}"
       curState = zonedevice.currentValue("motion")
    }
    else
    {
       curState = zonedevice.currentValue("contact")
       newState = "${zonestatus}"
    }
    
    log.debug "Zone Device: $zonedevice.displayName ($zonedevice.deviceNetworkId) is ${newState} : $newState = $curState"
   
    if( "$curState" != "${newState}" )
    {
       log.debug "Sending update - Zone Device: $zonedevice.displayName ($zonedevice.deviceNetworkId) is ${newState}"
       zonedevice.zone("${zonestatus}")
    }
//  notifyEvents.each { notifyEvent->
//    if( false && notifyEvent == "zone ${zonestatus}" ) {
//      def ntfMsg = "Zone Device: $zonedevice.displayName ($zonedevice.deviceNetworkId) is ${zonestatus}"
//      sendMsg( ntfMsg )
//    }
//  }
  }
...
private updatePartitions(paneldevices, partitionnum, partitionstatus) {
  ...
  if (paneldevice) {
    ...
    def curState = paneldevice.currentValue("dscpartition")
    if( "$curState" != "${partitionstatus}" )
    {
       log.debug "Sending update - Panel device: $paneldevice.displayName ($paneldevice.deviceNetworkId) is ${partitionstatus}"
       paneldevice.partition("${partitionstatus}", "${partitionnum}")
    }
//    notifyEvents.each { notifyEvent->
//      if( notifyEvent == "all" || notifyEvent == "${partitionnum}" ) {
//        def ntfMsg = "Panel device: $paneldevice.displayName ($paneldevice.deviceNetworkId) is ${partitionstatus}"
//        sendMsg( ntfMsg )
//      }
//    }
  }
