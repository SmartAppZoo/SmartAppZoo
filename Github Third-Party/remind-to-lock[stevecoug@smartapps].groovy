...
* Matt (@mattjfrank).
* Modified extensively by SM Consulting, LLC
*
* --------------------------------------------------------------------------
*
...
definition(
	...
	description: "Notify me when a door is left unlocked.",
	category: "Convenience",
	iconUrl: "http://www.veryicon.com/icon/png/Object/Sigma%20General/lock.png",
	iconX2Url: "http://www.veryicon.com/icon/png/Object/Sigma%20General/lock.png"
)

preferences {
	section {
		paragraph "This smart app monitors a door lock and sends push notifications, text messages and/or voice notifications if the door is left unlocked for longer than a specified period of time."
		paragraph "Version 2.0.0\nCopyright (c) Statusbits.com and SM Consulting, LLC"
	}
	section("Remind me when a lock is unlocked") {
		input "lock", "capability.lock", title:"Which lock?"
		input "notifyAfter", "number", title:"If unlocked for (minutes)", defaultValue:5
...
private def scheduleTask() {
	TRACE("scheduleTask()")
	def currentTime = now()
	state.timeUnlocked = currentTime
	state.repeatCount = 0
...
def onUnlocked(evt) {
	TRACE("onUnlocked(${evt.displayName})")
	if (state.notifyMs) {
		scheduleTask()
	}
}
...
def taskNotify() {
	...
	def currentTime = now()
	def unlockedFor = Math.round((currentTime - state.timeUnlocked) / 60000)
	def message = "The ${lock.displayName} has been unlocked for ${unlockedFor} "
	if (unlockedFor > 1) {
		message += "minutes."
	} else {
		message += "minute."
