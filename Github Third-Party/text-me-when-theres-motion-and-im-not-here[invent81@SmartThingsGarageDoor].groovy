	...
	if (presence1.latestValue("presence") == "not present") {
		...
		if (alreadySentSms) {
            ...
            if (location.contactBookEnabled) {
                log.debug "$motion1 has moved, sending notifications to: ${recipients?.size()}"
                sendNotificationToContacts("${motion1.label} ${motion1.name} moved", recipients)
            }
            else {
                log.debug "$motion1 has moved, sending text"
                sendSms(phone1, "${motion1.label} ${motion1.name} moved")
            }
		}
	} else {
