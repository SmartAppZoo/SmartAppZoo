...
def initialize() {
	...
	if ((settings.masterSwitchId == null) || !settings.masterOnly) {
		...
		subscribe(switches, 	"colorTemperature", colorTemperatureHandler)
		subscribe(switches, 	"colorMode", 		colorModeHandler)
		...
		subscribe(masterSwitch, "colorTemperature", colorTemperatureHandler)
		subscribe(masterSwitch, "colorMode", 		colorModeHandler)
	}
...
def colorModeHandler(evt) {
	syncColorModeState(evt.deviceId)
}
...
def syncColorModeState(triggeredDeviceId) {
	long now = (new Date()).getTime()
	...
	def triggeredDevice = switches.find { it.deviceId == triggeredDeviceId }
	def newColorMode = triggeredDevice.hasAttribute('colorMode') ? triggeredDevice.currentValue("colorMode", true) : null
	
    if (newColorMode == null) {
        return
    }

  	log "BINDING: ${triggeredDevice.displayName} COLORMODE ${newColorMode} detected"
	...
	// Push the event out to every switch except the one that triggered this.
	switches.each { s -> 
		if ((s.deviceId != triggeredDeviceId) && s.hasCommand('setColorMode')) {
			if (s.currentValue('colorMode', true) != newColorMode) {
				log "BINDING: ${s.displayName} -> setColorMode($newColorMode)"
				s.setColorMode(newColorMode)
			} else {
				log "BINDING: ${s.displayName} is already at colormode $newColorMode"
			}
		} else {
            if (s.deviceId != triggeredDeviceId) {
                log "BINDING: ${s.displayName} does not support setColorMode()"	
            }
		}
	}
}
