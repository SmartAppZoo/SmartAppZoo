...
def notifyPage() {
	dynamicPage(name: "notifyPage", title: "Configure Notifications", uninstall: false) {
		...
		section("Smart Message Control Support:") {
        	input "smc", "bool", title: "Send messages and reports to Smart Message Control App", required: false, submitOnChange: true,
...
//    sendLocationEvent(name: "SmartMessaging", value: "${app.label}", isStateChange: true, descriptionText: "${eTxt}")
    ...
    log.trace "event information sent to speaker control: $eTxt"
        	...
        	if (echoDevice) {
            log.info "Sending message to echoDevice: $echoDevice --> $eTxt"
            	echoDevice?.speak(eTxt)
}
	if (settings?.mySpeechDevices) {
