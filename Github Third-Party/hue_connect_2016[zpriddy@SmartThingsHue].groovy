...
def bulbDiscovery() {
  ...
  def groupoptions = groupsDiscovered() ?: [:]
  def numFoundGroups = groupoptions.size() ?: 0
  ...
  return dynamicPage(name:"bulbDiscovery", title:"Bulb Discovery Started!", nextPage:"", refreshInterval:refreshInterval, install:true, uninstall: true) {
    section("Please wait while we discover your Hue Bulbs. Discovery can take five minutes or more, so sit back and relax! Select your device below once discovered.") {
      ...
      input "selectedGroups", "enum", required:false, title:"Select Hue Groups (${numFoundGroups} found)", multiple:true, options:groupoptions
    }
    section {       
...
//This is a very beta function - Use with caution
private discoverHueSensors() {
  def host = getBridgeIP()
  sendHubCommand(new physicalgraph.device.HubAction([
    method: "GET",
    path: "/api/${state.username}/sensors",
    headers: [
      HOST: host
    ]], "${selectedHue}"))
}
...
def groupListHandler(hub, data = "") {
  def msg = "Groups list not processed. Only while in settings menu."
  def groups = [:]
  if (state.inBulbDiscovery) {       
    def logg = ""
    log.trace "Adding bulbs to state..."
  ...
  msg = "${groups.size()} groups found. ${groups}"
  return msg
}
  ...
  selectedGroups?.each { dni ->
    ...
    if(!d) {
      ...
      if (groups instanceof java.util.Map) {
        ...
        if (newHueGroup != null) {
          if (newHueGroup?.value?.type?.equalsIgnoreCase("LightGroup") ) {
  ...
  if (parsedEvent?.ssdpTerm?.contains("urn:schemas-upnp-org:device:basic:1")) {
    ...
    if (headerString?.contains("xml")) {
      ...
      if (body.success != null) {
        ...
        if (!body?.action) { //check if first time poll made it here by mistake
          def bulbs = getHueBulbs()
          def groups = getHueGroups()
  ...
  if (parsedEvent.headers && parsedEvent.body) {
    ...
    if (headerString?.contains("json")) { 
      ...
      if (body instanceof java.util.HashMap) { 
        ...
        // Parse Groups
        def groups = getChildDevices()
        for (group in body) {
          def d = groups.find{it.deviceNetworkId == "${app.id}/${bulb.key}g"}    
          if (d) {
            if(group.value.type == "LightGroup") {
              sendEvent(d.deviceNetworkId, [name: "switch", value: group.value?.state?.on ? "on" : "off"])
              sendEvent(d.deviceNetworkId, [name: "level", value: Math.round(group.value.state.bri * 100 / 255)])
              if (group.value.state.sat) {
                def hue = Math.min(Math.round(group.value.state.hue * 100 / 65535), 65535) as int
                def sat = Math.round(group.value.state.sat * 100 / 255) as int
                def hex = colorUtil.hslToHex(hue, sat)
                sendEvent(d.deviceNetworkId, [name: "color", value: hex])
                sendEvent(d.deviceNetworkId, [name: "hue", value: hue])
                sendEvent(d.deviceNetworkId, [name: "saturation", value: sat])
...
def on(childDevice, transition=4, type="bulb") {
  log.debug "Executing 'on' - $type"
  transition = transition * 10
  if(type == "bulb"){
    put("lights/${getId(childDevice)}/state", [on: true, transitiontime:transition])
  }
  if(type == "group"){
    put("groups/${getId(childDevice)}/action", [on: true, transitiontime:transition])
  }
  return "Bulb is On"
}

def off(childDevice, transition=4, type="bulb") {
  log.debug "Executing 'off' - $type"
  transition = transition * 10
  if(type == "bulb"){
    put("lights/${getId(childDevice)}/state", [on: false, transitiontime:transition])
  }
  if(type == "group"){
    put("groups/${getId(childDevice)}/action", [on: false, transitiontime:transition])
  }
  return "Bulb is Off"
}

def setLevel(childDevice, percent, transition=4, type="bulb") {
  log.debug "Executing 'setLevel' - $type"
  transition = transition * 10
  def level 
  if (percent == 1) level = 1 else level = Math.min(Math.round(percent * 255 / 100), 255)
  if(type == "bulb")
  {
    put("lights/${getId(childDevice)}/state", [bri: level, on: percent > 0, transitiontime:transition])
  }
  if(type == "group")
  {
    put("groups/${getId(childDevice)}/action", [bri: level, on: percent > 0, transitiontime:transition])
  }
}

def setSaturation(childDevice, percent, transition=4, type="bulb") {
  log.debug "Executing 'setSaturation($percent)' - $type"
  transition = transition * 10
  def level = Math.min(Math.round(percent * 255 / 100), 255)
  if(type == "bulb")
  {
    put("lights/${getId(childDevice)}/state", [sat: level, transitiontime:transition])
  }
  if(type == "group")
  {
    put("groups/${getId(childDevice)}/action", [sat: level, transitiontime:transition])
  }
}

def setHue(childDevice, percent, transition=4, type="bulb") {
  log.debug "Executing 'setHue($percent)'"
  def level = Math.min(Math.round(percent * 65535 / 100), 65535)
  if(type == "bulb")
  {
    put("lights/${getId(childDevice)}/state", [hue: level, transitiontime:transition])
  }
  if(type == "group")
  {
    put("groups/${getId(childDevice)}/action", [hue: level, transitiontime:transition])
  }
}

def setColor(childDevice, huesettings, type="bulb") {
  ...
  def sat = Math.min(Math.round(huesettings.saturation * 255 / 100), 255)
    ...
    def transition = huesettings.transition ? huesettings.transition * 10 : 4
...
def nextLevel(childDevice, type="bulb") {
  ...
  setLevel(childDevice,level, type)
}
...
def convertZGroupListToMap() {
  try {
    if (state.groups instanceof java.util.List) {
      def map = [:]
      state.groups.unique {it.id}.each { group ->
        map << ["${group.id}":["id":group.id+"g", "name":group.name, "hub":group.hub]]
      }
      state.group = map
    }
  }
