 ...
 *
 *  Fibaro RGBW Night Mode
 *
 *  Author: Max Petyurenko
 */

definition(
    name: "Fibaro RGBW Night Mode",
    namespace: "Fibaro",
    author: "Max Petyurenko",
    description: "Enables/disables night mode for Fibaro RGBW during specified period.",
...
preferences {
  section('Control this Fibaro RGBW') {
    input 'fibaroRGBWDevices', 'capability.colorControl', multiple: true, required: true
  }
...
private enableNightMode() {
  if (fibaroRGBWDevices) {
    log.debug 'Enable night mode for Fibaro RGBW devices'

    fibaroRGBWDevices*.changeSingleParameter(12, 1,  Math.round(reducedBrightness * 255 / 100))
  }
}
...
private disableNightMode() {
  if (fibaroRGBWDevices) {
    log.debug 'Disable night mode for Fibaro RGBW devices'
    fibaroRGBWDevices*.changeSingleParameter(12, 1, 255)
  }
}