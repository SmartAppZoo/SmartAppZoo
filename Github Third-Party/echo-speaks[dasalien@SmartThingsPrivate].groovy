...
def actionsPage() {
    return dynamicPage(name: "actionsPage", nextPage: "mainPage", uninstall: false, install: false) {
        ...
        section() {
            app(name: "actionApp", appName: actChildName(), namespace: "acrosscable12814", multiple: true, title: inTS("Create New Action", getAppImg("es_actions", true)), image: getAppImg("es_actions"))
        ...
        section() {
            if(state?.actionDuplicated) {
                ...
                if(act) {
                    ...
                    addChildApp("acrosscable12814", actChildName(), "${actData?.label} (Dup)", [settings: actData?.settings])
        ...
        section() {
            if(state?.zoneDuplicated) {
                ...
                if(zn) {
                    ...
                    addChildApp("acrosscable12814", zoneChildName(), "${znData?.label} (Dup)", [settings: znData?.settings])
...
def zonesPage() {
    return dynamicPage(name: "zonesPage", nextPage: "mainPage", uninstall: false, install: false) {
        ...
        section() {
            app(name: "zoneApp", appName: zoneChildName(), namespace: "acrosscable12814", multiple: true, title: inTS("Create New Zone", getAppImg("es_groups", true)), image: getAppImg("es_groups"))
            if(zApps?.size() && isST()) {
        ...
        if (evtData?.keySet()?.size()) {
            ...
            if (evtData?.echoDevices?.size()) {
                    ...
                    if (!childDevice) {
                        ...
                        if(settings?.autoCreateDevices != false) {
                            try{
                                ...
                                childDevice = addChildDevice("acrosscable12814", childHandlerName, dni, null, [name: childHandlerName, label: devLabel, completedSetup: true])
                            } catch(ex) {
                ...
                if(!isST()) {
                    ...
                    if(!wsDevice) { addChildDevice("acrosscable12814", wsChildHandlerName, "${app?.getId()}|echoSpeaks_websocket", null, [name: wsChildHandlerName, label: "Echo Speaks - WebSocket", completedSetup: true]) }
