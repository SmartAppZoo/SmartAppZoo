...
def otherSettings() {
	dynamicPage(name: "otherSettings", title: "Other Settings", install: true, uninstall: false) {
		section("Polling at which interval in minutes (range=[5,10,15,30],default=5 min.)?") {
...
def initialize() {
	...
	if ((delay < 5) || (delay>30)) {
		msg= "MyEcobeeInit>scheduling interval not in range (${delay} min), exiting..."
		log.debug msg
		send msg        
 		return
	}
