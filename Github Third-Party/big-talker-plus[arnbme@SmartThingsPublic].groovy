/**  
 *  Nov 27, 2017  ability to do delays on multiple or single commands (find code at cmd_delay)
 *					Note: Tested with Lannouncer only, no real speakers available on my system
 *					Phrases are parsed into sub_phrases by either ? (question mark) or & (ampersand) character 
 *					Delays immediately follow command with ~~nnn(nn) where nnn is milliseconds of delay, length 3 to 5 digits
 *					Subsequent delays are added to exising delay
 *					Phrases with no delays process as a single command (Original default)
 *					Example: hello&bye~~2000?bye again~~2000  
 *						"hello" speaks immediate
 *						"bye" speaks at +2 seconds
 *						"bye again" speaks at +4 seconds
 *					Example: @|ALARM=CHIME~~2000&Please enter your pin code on the keypad~~2000?@|ALARM=CHIME~~3000  
 *						Chime sounds at +2 seconds
 *						"Please enter..." speaks at +4 seconds
 *						Chime sounds at +7 seconds
 *  BIG TALKER -- Version 1.1.12 -- A SmartApp for SmartThings Home Automation System
...
//				log.debug "${phrase} ${commands} ${commands.length}"
				if (matcher != phrase =~ mask)
					...
					commands.each
						{ cmd ->
						if (cmd<" ")
							{log.debug "empty command skipped"}
						else
							{
							if((matcher = cmd =~ mask))
								{
...
//								log.debug "${matcher[0][2]}"	//the delay
								cmd_delay+=matcher[0][2].toInteger()	//increment existing delay time
//								log.debug "updated delay ${cmd_delay}"
								it.speak(matcher[0][1], [delay: cmd_delay])
								}
							else
							if (cmd_delay>0)
								{
//								log.debug "Existing delay ${cmd} ${cmd_delay}"
								it.speak(cmd, [delay: cmd_delay])
								}				
							else
								{
//								log.debug "No delay"
