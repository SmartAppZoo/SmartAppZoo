 ...
 *
 *	Iris Smart Button Controller
 *
 *	Author: Doug Hill
 *	Date: 2016-6-1
 */
definition(
    name: "Iris Smart Button Controller",
    namespace: "thedoughill",
    author: "Doug Hill",
    description: "Control Iris Smart Button (2nd Gen)",
...
def buttonEvent(evt){
	if(allOk) {
		...
		def value = evt.value
        def buttonLabel = buttonDevice.displayName
		...
		if(recentEvents.size <= 1){
			executeHandlers(1, buttonLabel, value)
		} else {
...
def executeHandlers(buttonNumber, buttonLabel, value) {
	...
	if (notifications?.toBoolean()) sendPush(textMessage ?: "$buttonLabel was pressed" )

	def phone = find('phone', buttonNumber, value)
	if (phone != null) sendSms(phone, textMessage ?:"$buttonLabel was pressed")
