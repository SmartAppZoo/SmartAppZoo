/**
 *  Virtual Motion Sensor with 1 conditional switch - Ver 1.0
...
definition(
    name: "Virtual Motion Sensor with 1 conditional switch",
    namespace: "Steve Jackson",
    author: "Steve Jackson, SmartThings",
...
preferences {
	...
	section("Do Not Disturb Switch (MUST BE ON to disable)?"){
		input "dndswitch", "capability.switch", multiple: false
	}	
...
def motionHandler(evt) { 
	def dndswstate = dndswitch.currentSwitch
    def motionstate = motion1.currentMotion
    log.debug "Current Do Not Disturb Switch State: $dndswstate"
...
}	else if (dndswstate == "on") {
		log.debug "No Change to Simulated Motion Sensor, DND switch is ON"
	}
}
