 ...
 * 2015-9-25 Set the smartapp to only run if a switch is on.
 */
definition(
    name: "Cat Arrived Notifier",
...
preferences {
    ...
    section("Enable Switch") {
    	input "enablecontact", "capability.contactSensor", title: "Turn on contact", multiple: false, required: false
...
def subscribeToEvents() {
    ...
    subscribe(enablecontact, "contact", switchHandler)
...
def switchHit(evt) {
   ...
   // def current = theSensor.currentState("contact")
    ...
    log.debug "Notification from switch: ${state.NotificationEnabled}"
}


/* def switchHandler(evt) {
	def somethingclosed = settings.enablecontact?.find { it.currentContact == 'closed' }
    log.debug "somethingclosed ${somethingclosed}"
    if (somethingclosed) {
    	state.NotificationEnabled = true;
    } else {
    	state.NotificationEnabled = false;
    }
    log.debug "Notification from contact: ${state.NotificationEnabled}"
} */


def eventHandler(evt) {
	log.debug "Notify got evt ${evt}"
    log.debug "enableSwitch ${state.NotificationEnabled}"
