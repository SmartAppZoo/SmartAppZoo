...
definition(
    ...
    namespace		: "Echo",
    author			: "JH/BD",
	description		: "Logic Blockz - The power behind LogicRulz",
	category		: "My Apps",
    parent			: "Echo:LogicRulz",
...
/**********************************************************************************************************************************************/
private def version() { 
    	def text = "Logic Blockz Ver 2.0 / R.0.4.3, Release date: 09/13/2018, Initial App Release Date: 04/23/2018" 
	}
    ...
    dynamicPage(name: "mainProfilePage", title:"", install: true, uninstall: installed) {
        section ("Details and Status") {
            label title: "Name this Logic Block", required:true
            	...
            	input "rPause", "bool", title: "Block Status: On = Running, Off = Paused", defaultValue: false, submitOnChange: true
            }
        if (rPause == null) {
        	section ("") {
        ...
        section(""){
    	    ...
    	    section ("Logic Block Configuration") {
                ...
                href "actions", title: "Actions", description: pDevicesComplete(), state: pDevicesSettings()
   		        href "pSend", title: "Audio, Push, & SMS messages", description: pMessageComplete(), state: pMessageSettings()   
            }
        }    
    }
}
...
def actions() {
    ...
    if (actions.contains("Mode, SHM, Routines, Logic Blocks & RemindR Profiles")) {
        ...
        if (actions.contains("Custom Commands")) {
            section ("Custom Commands", hideable: true, hidden: false) {
                input "custDevice", "capability.sensor", title: "Run Custom Commands on these devices", required: false, multiple: true, submitOnChange: true
                if (custDevice) {
                    def cmdList = []
                    custDevice.each {cap ->
                        cap.supportedCommands.each {comm ->
                            String command  = (String) comm.name
                            cmdList += command
                            state.custCmd = []
                            cmdList.findAll {state.custCmd << "$it"}
                        }    
                    }
                    input "custCommand", "enum", title: "Supported commands for $custDevice", options:  state.custCmd, required: false, multiple: false, submitOnChange: true
                    def listSize = custDevice.size()
                    if (listSize > 1) {
                        paragraph "The provided list of commands is a combination of all selected devices. Some commands may work on one device but not others"
                    }
                    if (custCommand) {
                        if (!schedcustCmd) input "custCmdOnExe", "bool", title: "$custCommand when this Logic Block Executes", default: false, required: false, submitOnChange: true
                        if (custCmdOnExe) input "custCmdDelay", "number", title: "Delay running by this number of seconds", defaultValue: 0, required: false, submitOnChange: true
                        if (!custCmdOnExe) input "schedcustCmd", "bool", title: "Schedule $custDevice to $custCommand every...", required: false, default: false, submitOnChange: true
                        if (schedcustCmd) input "runEvery", "enum", title: "Run every....", options: ["1 Minute", "5 Minutes", "10 Minutes", "15 Minutes", "30 Minutes"]
                    }
                } 
            }        
        }
	if (actions.contains("Device Health")) {
	section ("Device Health") {
    	input "deviceHealth", "capability.sensor", title: "select a device to monitor", multiple: true, submitOnChange: true, required: false
        if (deviceHealth) {
        deviceHealth.each { device ->
        paragraph "The device ${device.displayName} was last ${device.getStatus()} on ${device.getLastActivity()}" //\n" +
        def theStates = device.statesSince("contact", new Date() -1)
		def between = timeOfDayIsBetween(new Date() - 1, new Date() + 1,
        	new Date(), location.timeZone)
            paragraph "There are ${theStates.size()} event records between $between"
}
        //deviceHealth.eventsSince(new Date() - 1, [max: 1]).descriptionText
			}
        }
	}
}
