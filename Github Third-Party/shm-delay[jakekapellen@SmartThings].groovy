...
//				title: "The upgraded Keypad module is installed Default: Off/False"
			...
			if (globalKeypadControl)
				...
				actions?.sort()
				input "globalKeypadDevices", "device.IrisKeypad", required: false, multiple: true,
...
//	atomicState.badpins=0		//reset badpin count
	...
	if (modeEntered==3 && globalKeypadExitDelay > 0)	//in away mode process Exit Delay when requested
            ...
            //JMK TODO: clean this up! I shouldn't have to manually trigger this!
            findAllChildAppsByName('SHM Delay Child').each
            	{ delayChild ->
...
//											  not ideal prefer alarmtime but its before new alarm time is set
	...
	if (armMode == 'Away')
        ...
        findAllChildAppsByName("SHM Delay Child").each
        	{ app ->
                def locevent = [name:"shmdelaytalk", value: "exitAway", isStateChange: true,
                    displayed: true, descriptionText: "Finish exit delay", linkText: "Issue exit delay talk event",
                    data: globalKeypadExitDelay]
            	app.delayTalkHandler(locevent)
            }
		} 
