 ...
 *
 *  SmartUY Sonoff Connect
 *
 */

definition(
    name: "SmartUY Sonoff Connect",
    namespace: "SmartUY",
    author: "SmartUY",
    description: "Service Manager for Sonoff switches",
    category: "Convenience",
    iconUrl:   "https://raw.githubusercontent.com/bigjuanpa/JPLInnovations/master/sonoff-connect-icon.png",
    iconX2Url: "https://raw.githubusercontent.com/bigjuanpa/JPLInnovations/master/sonoff-connect-icon-2x.png",
    iconX3Url: "https://raw.githubusercontent.com/bigjuanpa/JPLInnovations/master/sonoff-connect-icon-3x.png"
)
...
preferences {
    ...
    page(name: "discoveryPage", title: "Device Discovery", content: "discoveryPage", refreshTimeout:3)
...
def mainPage() {
	dynamicPage(name: "mainPage", title: "Manage your switches", nextPage: null, uninstall: true, install: true) {
        section("Configure"){
...
def manuallyAdd(){
   dynamicPage(name: "manuallyAdd", title: "Manually add a device", nextPage: "manuallyAddConfirm") {
		section {
...
def manuallyAddConfirm(){
   if ( ipAddress =~ /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/) {
       ...
       addChildDevice("JPL", deviceType ? deviceType : "Sonoff JP Wifi Switch", "${convertIPtoHex(ipAddress)}:${convertPortToHex("80")}", location.hubs[0].id, [
           "label": (deviceType ? deviceType : "Sonoff JP Wifi Switch") + " (${ipAddress})",
           "data": [
           "ip": ipAddress,
           "port": "80" 
           ]
...
void deviceDescriptionHandler(physicalgraph.device.HubResponse hubResponse) {
	...
	if (body?.device?.modelName?.text().startsWith("JPL")) {
		def devices = getDevices()
