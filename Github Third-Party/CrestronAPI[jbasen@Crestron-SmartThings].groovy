...
void updateThermostat() {
  ...
  else if(command == 'getthermostatstatus'){
  	log.debug "Thermostat Status Requested"
    SendUpdate("/${device.id}/=CoolSP=${device.currentValue("coolingSetpoint")}")
    SendUpdate("/${device.id}/=HeatSP=${device.currentValue("heatingSetpoint}")}")
    SendUpdate("/${device.id}/=SystemMode=${device.currentValue("thermostatMode")}")
    SendUpdate("/${device.id}/=FanMode=${device.currentValue("thermostatFanMode")}")
    SendUpdate("/${device.id}/=temperature=${device.currentValue("temperature")}")
  	}
    device.poll()
...
private SendUpdate(String path) {
    def pollParams = [
        uri: "http://TODO.mycrestron.com:PORT",
        path: "${path}"
        ]
	httpGet(pollParams)
}
