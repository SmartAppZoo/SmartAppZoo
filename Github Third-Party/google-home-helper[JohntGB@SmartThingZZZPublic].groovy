...
definition(
    ...
    iconX3Url: "https://raw.githubusercontent.com/JohntGB/SmartThingZZZPublic/master/smartapps/michaelstruck/google-home-helper.src/GoogleHome@2x.png")
preferences {
    page name:"pageMain"
    //Parent menu pages
...
def pageAbout(){
	dynamicPage(name: "pageAbout", uninstall: true) {
		section {
        	paragraph "${textAppName()}\n${textCopyright()}", image: "https://raw.githubusercontent.com/JohntGB/SmartThingZZZPublic/master/smartapps/michaelstruck/google-home-helper.src/GoogleHome@2x.png"
        }
...
def speakerVolHandler(evt){
    if (getOkToRun("Speaker volume change")) {
    	...
    	if (speakerLevel == 0) vDimmerSpeaker.off()
          ...
          if (upLimitSpeaker && lowLimitSpeaker) {
              speakerLevel = Math.floor(lowLimitSpeaker + (((upLimitSpeaker - lowLimitSpeaker) * speakerLevel) / 100))
          } else if (upLimitSpeaker && (vDimmerSpeaker.currentValue("level") > upLimitSpeaker)) {
              speakerLevel = upLimitSpeaker
          } else if (lowLimitSpeaker && (vDimmerSpeaker.currentValue("level") < lowLimitSpeaker)) {
...
def getAlarmSound(){
    ...
    def soundUri = [uri: "https://raw.githubusercontent.com/JohntGB/SmartThingZZZPublic/master/smartapps/michaelstruck/alexa-helper-scenario.src/AlarmSirens/AlarmSiren${alarmSonosSound}.mp3", duration: "${soundLength}"]
